...
| `site/dados/` | Gerado, fora do git. É o retrato local para desenvolver. Dentro dele: `uf/`, `cand/` (uma ficha por candidatura), `forn/` (as fichas de fornecedor, em blocos), `forn-recorte/` (quem recebeu, por unidade e por partido e cargo), `indice.json`, `alarmes.json`, `cota.json`, `meta.json` |
