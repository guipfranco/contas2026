...
**O tipo por candidatura no país mora em arquivo à parte, e é baixado sob demanda** `[decidido, Guilherme 18/09/2026]`. O campo 11 da linha de `uf/BRASIL.json` continua vazio: a lista de tipos era 36 % daquele arquivo, quase 1 MB comprimido que o celular baixava para alimentar um painel que nasce fechado. O mesmo dado existe em `tipos/BRASIL.json`, e quem o busca é a visão geral ou o filtro de tipo. Com ele carregado o front cruza partido, cargo, estado, tipo e candidatura sozinho, no país inteiro, como já faz nas UFs.
...
| `site/dados/` | Gerado, fora do git. É o retrato local para desenvolver. Dentro dele: `uf/`, `cand/` (uma ficha por candidatura), `forn/` (as fichas de fornecedor, em blocos), `forn-recorte/` (quem recebeu, por unidade e por partido e cargo), `forn-cruzado/` (quem recebeu por tipo de despesa, e a divisão por partido dos maiores), `tipos/BRASIL.json` (o gasto por tipo de cada candidatura do país), `indice.json`, `alarmes.json`, `cota.json`, `meta.json` |
...
- **A tela é uma só, com cinco dimensões da mesma lista** `[decidido, Guilherme 18/09/2026]`: candidato, partido, estado, cargo e fornecedor. Só a dimensão vai no endereço (`v=`); os filtros são os mesmos nas cinco e **sobrevivem à troca**, e o filtro que não recorta uma delas fica na barra desligado, mostrando o valor guardado. Partido, estado e cargo agrupam **no front**, sobre as linhas que `passa()` já filtrou, e **não mostram sinal nem índice**: quem presta contas é a candidatura, e um índice por partido seria nota dada a um partido pela soma de gente. Endereço antigo (`#sinais`, `#panorama`, `#fornecedores`) é traduzido na entrada, em `mapaAntigo`.
- **A sexta entrada da barra é cruzamento, e não uma dimensão** `[decidido, Guilherme 18/09/2026]`. `v=geral` mostra os cinco eixos ao mesmo tempo, em cartões de barras que **filtram uns aos outros pelos mesmos chips**: tocar numa barra liga o filtro que já existe, e o recorte sobrevive à troca de aba como em qualquer outra. Embaixo, dois fluxos de duas colunas, do partido para as maiores candidaturas e do partido para os maiores fornecedores, cada um com nó "outros" de conta exata e tabela irmã. Quatro regras dela: **uma medida só na tela inteira**, o gasto contratado no recorte (ou o gasto no tipo escolhido), porque misturar receita num cartão e gasto no vizinho faria a mesma tela carregar dois eixos com o mesmo rótulo; **nenhuma cor por partido**, aqui como em toda parte, e quem identifica é o nome ao lado; **sinal e federação desligados**, porque os cartões de fornecedor não sabem honrá-los; **sem índice em lugar nenhum dela**. O nó "outros" costuma ser a maior parte do dinheiro e entra **fora de escala**, com altura limitada a 30 % do desenho e o rótulo dizendo isso: em escala real, as 10 maiores candidaturas de Roraima são 30 % do contratado e os dez nomes saíam numa faixa de 12 px. **A escala do fluxo é uma só para as duas colunas enquanto ela couber** `[decidido, Guilherme 19/09/2026]`: com a mesma escala dos dois lados a faixa entra e sai com a mesma espessura, e é isso que vale sempre que o menor nó listado ainda ganhar 13 px nela. Quando não ganha, cada coluna passa a ter a sua, e a legenda embaixo do desenho diz qual dos dois casos vale. Medido no país: os dez maiores fornecedores somam R$ 10 mi de R$ 69 mi, e na escala da coluna dos partidos eles sairiam numa faixa de 60 px para dez nomes, então esse fluxo cai no segundo caso. **O desenho tem altura de tela, e não de rolagem:** ele cabe em 520 px, e é o teto do nó "outros", agora em 32 %, que paga por isso.
- **A visão geral é a tela que abre, e o alinhamento dela é por corte** `[decidido, Guilherme 19/09/2026]`. Endereço sem `v=` significa `geral`, e quem escreve `v=candidato` agora é a lista. Cada cartão tem lugar marcado na grade: a coluna fina empilha cargos e estados, partidos e as maiores candidaturas descem as duas linhas dela, e embaixo a fonte do dinheiro fica sobre os tipos de despesa, com os maiores fornecedores ao lado. Cartões que dividem linha terminam na mesma altura, e **quem se ajusta é a lista comprida**, escondendo linhas para o "outros N" do pé dela: esticar o cartão curto com ar faz o vizinho parecer ter mais conteúdo do que tem. A conta mede no navegador, repete no máximo quatro vezes e roda **antes** de desenhar os dois fluxos, que são a parte cara. Ela tem **piso de oito linhas**, senão uma tela de unidade única, onde a coluna fina é curta, derrubaria as listas dos vizinhos para quatro nomes. O título de cada cartão é o endereço da aba daquela dimensão, com o filtro de agora junto; tipos de despesa e a fonte do dinheiro não têm aba e ficam sem link.
- **O cartão de estados numa unidade só não traz medida de outra unidade** `[decidido, Guilherme 19/09/2026]`. No Brasil ele tem uma barra por unidade, na medida do recorte. Numa UF, a tela leu um arquivo só: a unidade da tela sai em evidência, com a medida do recorte, e as outras entram como porta para trocar de tela, sem barra e sem valor, com a nota dizendo por quê. O total de cada unidade existe no meta, mas é o total sem filtro, e pô-lo em barra ao lado dos outros cartões faria a mesma tela carregar dois eixos com o mesmo rótulo.
- **Quem recebeu não se agrupa no front, e por isso existe `forn-recorte/`.** A linha do ranking não diz quem recebeu, e as 419.531 fichas não cabem no aparelho de ninguém: o pipeline grava um arquivo por unidade com a lista geral, uma por partido, uma por cargo e uma por célula partido+cargo, cada uma com o topo e **quantos ficaram de fora somando quanto**. O partido entra pelo índice do dicionário do meta, o mesmo da linha do ranking. Sem corte o arquivo do país daria 34 MB, e `LIMITE_RECORTE_MB` no validador é a sentinela disso. **`tem_ficha` e o endereço seguem o total nacional do fornecedor, nunca o do recorte**, senão o mesmo nome abriria página numa tela e não na outra.
